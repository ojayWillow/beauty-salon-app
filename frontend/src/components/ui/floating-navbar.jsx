...
export function FloatingNav({ navItems, brandName = '✦ Sandra Beauty', lang = 'lv', onToggleLang, className }) {
    ...
    <>
      <motion.nav
        ...
        className={cn(
          ...
          scrolled ? 'bg-white/90 backdrop-blur-xl shadow-md border-b border-rose-100' : 'bg-transparent',
          className
        )}
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 clamp(1.5rem, 5vw, 4rem)', height: '72px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {/* Desktop nav links */}
          <ul className="nav-desktop-links" style={{ display: 'flex', alignItems: 'center', gap: '2.4rem', listStyle: 'none', margin: 0, padding: 0 }}>
            {navItems.filter(n => !n.isAction && !n.isCart).map((item, idx) => (
              <li key={idx}>
                <a
                  href={item.link}
                  style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-muted)', textDecoration: 'none', transition: 'color var(--transition)', paddingBottom: '4px' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            {navItems.filter(n => n.isCart).map((item, idx) => (
              <a key={idx} href={item.link} style={{ position: 'relative', display: 'flex', alignItems: 'center', color: 'var(--color-muted)' }}>
                ...
                {item.badge > 0 && (
                  <span style={{ position: 'absolute', top: '-8px', right: '-8px', background: 'var(--color-rose)', color: '#fff', fontSize: '0.65rem', fontWeight: 700, borderRadius: 'var(--radius-full)', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{item.badge}</span>
                )}
              </a>
            ))}
          ...
          >
            {navItems.filter(n => !n.isCart).map((item, idx) => (
              <a key={idx} href={item.link} onClick={() => setMobileOpen(false)} style={{ display: 'block', padding: '0.75rem 0', fontFamily: 'var(--font-sans)', fontSize: '0.9rem', fontWeight: 500, color: 'var(--color-charcoal)', textDecoration: 'none', borderBottom: '1px solid var(--color-border)' }}>
                {item.name || (item.isAction ? (lang === 'lv' ? 'Ieiet' : 'Sign In') : '')}
              </a>
            ))}
          </motion.div>
        )}
